...
type long_10 = [i64; 10];
const BASE_R2Y: long_10 = [5744, 8160848, 4790893, 13779497, 35730846, 12541209, 49101323, 30047407, 40071253, 6226132];
const BASE_2Y: long_10 = [39999547, 18689728, 59995525, 1648697, 57546132, 24010086, 19059592, 5425144, 63499247, 16420658];
...
fn mulaSmall(p: &[u8], q: &[u8], m: i64, x: &[u8], n: i64, z: i64) -> i64 {
    let mut v = 0;
    for i in 0..n {
        v += (q[i+m] as i64) + z*(x[i] as i64);
        p[i+m] = v as u8;
        v >>= 8
    }
    return v
...
fn mula32(p: &[u8], x: &[u8], y: &[u8], t: i64, z: i64) {
    let n: i64 = 31;
    let mut w: i64 = 0;
    for i in 0..t {
        let zy = z * (y[i] as i64);
        w += mulaSmall(p, p, i, x, n, zy) + (p[i+n] as i64) + zy*(x[n] as i64);
        p[i+n] = (w) as u8;
        w >>= 8;
    }
    p[t+n-1] = (w + (p[t+n-1] as i64)) as u8;
}

fn divmod(q: &[u8], r: &[u8], n: i64, d: &[u8], t: i64) {
    let mut rn = 0;
    let dt = (d[t-1] as i64) << 8;
    if t > 1 {
        dt |= d[t-2] as i64;
    }
    n -= 1;
    while n >= t-1 {
        let mut z = (rn << 16) | ((r[n] as i64) << 8);
        if n > 0 {
            z |= r[n-1] as i64;
        }
        z /= dt;
...
fn cpy(out: &mut long_10, input: &long_10) { // todo memcpy
    out[0] = input[0];
    out[1] = input[1];
    out[2] = input[2];
    out[3] = input[3];
...
fn set(out: &mut long_10, input: i64) {
    out[0] = input;
    out[1] = 0;
    out[2] = 0;
    out[3] = 0;
...
fn add(xy: &mut long_10, x: &long_10, y: &long_10) {
    xy[0] = x[0] + y[0];
    xy[1] = x[1] + y[1];
    xy[2] = x[2] + y[2];
    xy[3] = x[3] + y[3];
...
fn sub(xy: &mut long_10, x: &long_10, y: &long_10) {
    xy[0] = x[0] - y[0];
    xy[1] = x[1] - y[1];
    xy[2] = x[2] - y[2];
    xy[3] = x[3] - y[3];
...
fn mulSmall(xy: &mut long_10, x: &long_10, y: i64) {
    let mut t = (x[8] * y);
    xy[8] = (t & ((1 << 26) - 1));
    t = (t >> 26) + (x[9] * y);
    xy[9] = (t & ((1 << 25) - 1));
...
fn mul(xy: &mut long_10, x: &long_10, y: &long_10) {
...
fn sqr(x2: &mut long_10, x: &long_10) {
...
fn recip(y: &mut long_10, x: &long_10, sqrtassist: i64) {
    let mut t0: long_10 = [0; 10];
    let mut t1: long_10 = [0; 10];
    let mut t2: long_10 = [0; 10];
    let mut t3: long_10 = [0; 10];
    let mut t4: long_10 = [0; 10];
...
fn isNegative(x: &long_10) -> i64 { // todo obvious optimization
    let mut tmp = 0;
...
fn sqrt(x: &mut long_10, u: &long_10) {
    let mut v: long_10 = [0; 10];
    let mut t1: long_10 = [0; 10];
    let mut t2: long_10 = [0; 10];
...
fn montPrep(t1: &mut long_10, t2: &mut long_10, ax: &long_10, az: &long_10) {
    add(t1, ax, az);
    sub(t2, ax, az);
}

fn montAdd(t1: &mut long_10, t2: &mut long_10, t3: &long_10, t4: &long_10, ax: &mut long_10, az: &mut long_10, dx: &long_10) {
    mul(ax, t2, t3);
    mul(az, t1, t4);
    add(t1, ax, az);
    sub(t2, ax, az);
...
fn montDbl(t1: &mut long_10, t2: &mut long_10, t3: &long_10, t4: &long_10, bx: &mut long_10, bz: &mut long_10) {
    sqr(t1, t3);
    sqr(t2, t4);
    mul(bx, t1, t2);
    sub(t2, t1, t2);
...
fn xToY2(t: &mut long_10, y2: &mut long_10, x: &long_10) {
    sqr(t, x);
...
fn core(Px: &mut [u8], s: &mut [u8], k: &[u8], Gx: &[u8]) {
    let mut dx: long_10 = [0; 10];
    let mut t1: long_10 = [0; 10];
    let mut t2: long_10 = [0; 10];
    let mut t3: long_10 = [0; 10];
    let mut t4: long_10 = [0; 10];
    let mut x: [long_10; 2] = [[0; 10], [0; 10]];
    let mut z: [long_10; 2] = [[0; 10], [0; 10]];
    ...
    for i in (1..32).rev() {
        for j in (1..8).rev() {
            let bit1 = (k[i] as u64) >> (j as u64) & 1;
            let bit0 = !(k[i] as u64) >> (j as u64) & 1;
            let ax = &mut x[bit0];
            let az = &mut z[bit0];
            let bx = &mut x[bit1];
            let bz = &mut z[bit1];
...
//divmod((dstptr) t1, s, 32, order25519, 32);
        ...
        egcd32(s, &mut tmp2, &mut tmp3, s, &tmp1);
        if (s[31] & 0x80) != 0 {
...
pub fn Verify(Y: &mut [u8], v: &[u8], h: &[u8], P: &[u8]) {
    ...
    let mut p: [long_10; 2] = [[0; 10], [0; 10]];
    let mut s: [long_10; 2] = [[0; 10], [0; 10]];
    let mut yx: [long_10; 3] = [[0; 10], [0; 10], [0; 10]];
    let mut yz: [long_10; 3] = [[0; 10], [0; 10], [0; 10]];
    let mut t1: [long_10; 3] = [[0; 10], [0; 10], [0; 10]];
    let mut t2: [long_10; 3] = [[0; 10], [0; 10], [0; 10]];
    let mut vi: i64 = 0;
    let mut hi: i64 = 0;
    let mut di: i64 = 0;
    let mut nvh: i64 = 0;
    ...
    /* prepare the chain  */
    for i in 0..32 {
        vi = (vi >> 8) ^ (v[i] as i64) ^ ((v[i] as i64) << 1);
        hi = (hi >> 8) ^ (h[i] as i64) ^ ((h[i] as i64) << 1);
        nvh = !(vi ^ hi);
        di = (nvh & ((di & 0x80) >> 7)) ^ vi;
        di ^= nvh & ((di & 0x01) << 1);
        di ^= nvh & ((di & 0x02) << 1);
...
//            montAdd(&mut t1[1], &mut t2[1], &t1[k], &t2[k], &mut yx[1], &mut yz[1], p[di>>(j as u64)&1]);
//            montAdd(&mut t1[2], &mut t2[2], &t1[0], &t2[0], &mut yx[2], &mut yz[2], s[((vi^hi)>>(j as u64)&2)>>1]);

            k = ((vi ^ vi>>1) >> j & 1) + ((hi ^ hi>>1) >> j & 1);
            montDbl(&mut yx[2], &mut yz[2], &t1[k], &t2[k], &mut yx[0], &mut yz[0]);
            k = (di >> j & 2) ^ ((di >> j & 1) << 1);
            montAdd(&mut t1[1], &mut t2[1], &t1[k], &t2[k], &mut yx[1], &mut yz[1], p[di>>j&1]);
            montAdd(&mut t1[2], &mut t2[2], &t1[0], &t2[0], &mut yx[2], &mut yz[2], s[((vi^hi)>>j&2)>>1]);
        }
    }
...
pub fn IsCanonicalPublicKey(public_key: &[u8]) -> bool {
    let mut public_key_unpacked: long_10 = [0; 10];
    unpack(&mut public_key_unpacked, public_key);
    let public_key_copy: [u8; 32] = [0; 32];
...
fn egcd32(dest: &mut [u8], x: &mut [u8], y: &mut [u8], a: &[u8], b: &[u8]) {
    let mut an: i64 = 0;
    let mut qn: i64 = 0;
    let mut bn: i64 = 32;
